...
// Since it's a NextJS environment, requiring directly might fail.
// I will just copy the logic to a script to test the DB update.
...
async function run() {
  const { data: podcasts } = await supabase.from("podcasts").select("id, youtube_url, show_name").limit(3);
  if (!podcasts) return;
    ...
    if (endpoint) {
      ...
      if (res.ok) {
         ...
         if (data.items && data.items.length > 0) {
            ...
            console.log(`${podcast.show_name}: Last week views: ${pastSnapshots?.[0]?.total_views}, Current views: ${totalViews}, Delta: ${viewsLast7Days}`);
