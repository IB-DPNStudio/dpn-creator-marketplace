...
export function calculateHistoricalMetrics(
  ...
  // Keep snapshots that are older than 5 minutes (to ignore snapshots generated by the exact current refresh)
  // This allows intra-day rank changes to be reflected immediately on the dashboard.
  const pastSnapshots = sortedHistory.filter(h => {
    const snapshotTime = new Date(h.snapshot_date).getTime();
    return (now - snapshotTime) > 5 * 60 * 1000;
  });
  
  const lastWeekSnapshot = pastSnapshots.length > 0 ? pastSnapshots[0] : null; // Most recent valid past snapshot
